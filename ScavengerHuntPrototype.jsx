...
/* PICTURE PLACEHOLDER                                                    */
/* ---------------------------------------------------------------------- */
function PicturePlaceholder({ label, sublabel, tall = false, compact = false }) {
  return (
    <div
      className={
...
export default function ScavengerHuntPrototype() {
  ...
  const [screen, setScreen] = useState("intro"); // intro -> instructions -> hunt -> closing
    ...
    <div className="min-h-screen w-full bg-[#F1E9D8] text-[#2b2620] font-sans">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        {role === "camper" && screen === "intro" && (
          <IntroScreen onBegin={() => setScreen("instructions")} />
        )}
  ...
  return (
    <div className="space-y-5 quest-body">
      <Card className="text-center py-10">
        ...
        <PicturePlaceholder label="Title illustration" sublabel="a wall carved with heraldic signs, somewhere in the old quarter" tall={false} />
        <button
          onClick={onBegin}
  ...
  if (big) {
    return (
      <div className="space-y-3">
        <PicturePlaceholder label={`Photo of ${station.character}`} sublabel={station.meetLine} />
        <div>
        ...
        {phase === "resolved" && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <PicturePlaceholder label={station.character} sublabel="character" />
              </div>
              <div>
            ...
            {station.nextClueHint && (
              <div className="space-y-1.5">
                ...
                <PicturePlaceholder label={`Something at ${station.nextArea}`} sublabel={station.nextClueHint} />
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            {station.nextClueHint && (
              <div className="space-y-1.5">
                ...
                <PicturePlaceholder label={`Something at ${nextStation.name}`} sublabel={station.nextClueHint} />
              </div>
            )}
...
const MOCK_GROUPS = [
  { name: "Group A — Falcons", stationId: 6, badges: 2 },
  { name: "Group B — Lions", stationId: 11, badges: 4 },
  { name: "Group C — Owls", stationId: 2, badges: 0 },
];

function AdminView({ stationsData, updateStation, stationIdx, lettersCollected, badges, dragonCards, sword, iceCream, finaleDone }) {
  ...
  return (
    <div className="space-y-5 quest-body">
          ...
          <tbody>
            <tr className="border-b border-[#f1e9d8] bg-amber-50">
              <td className="py-2 font-semibold flex items-center gap-1">
                <Users className="w-3.5 h-3.5" /> Live demo group
              </td>
            ...
            {MOCK_GROUPS.map((g) => (
              <tr key={g.name} className="border-b border-[#f1e9d8]">
                <td className="py-2">{g.name}</td>
                <td className="py-2">Station {g.stationId}</td>
                <td className="py-2">{Math.round((g.stationId / stationsData.length) * 100)}%</td>
                <td className="py-2">{g.badges}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Card>
        ...
        <p className="text-xs text-[#8a7a5a] mb-3">Every photo a group submits, organized by group — for review or printing afterward.</p>
        <div className="space-y-3">
          {[{ name: "Live demo group", count: stationIdx + 1 }, ...MOCK_GROUPS.map((g) => ({ name: g.name, count: g.stationId }))].map((g) => (
            <div key={g.name}>
      ...
      <Card>
        ...
        <p className="text-sm text-[#8a7a5a] mb-3">Edit station text live — changes apply immediately to the camper experience (this demo session only; not yet persisted to a database).</p>
              ...
              {editingId === s.id ? (
                <div className="space-y-2.5">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={saveEdit} className="bg-emerald-700 text-white rounded-full px-4 py-1.5 text-sm font-semibold hover:bg-emerald-800">
                      Save
                    </button>
                    <button onClick={cancelEdit} className="border border-[#cfc09a] rounded-full px-4 py-1.5 text-sm hover:bg-[#e3d8bc]">
                      Cancel
                    </button>
                  </div>
                </div>
